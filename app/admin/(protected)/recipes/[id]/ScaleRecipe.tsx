...
export function ScaleRecipeButton({
  ...
  return (
    <>
      ...
      <dialog
        ref={dialogRef}
        className="m-auto max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border bg-card p-6 text-foreground backdrop:bg-black/40"
      >
        {scaleFactor == null ? (
                      ...
                      <tbody>
                        {group.ingredients.map((ingredient) => (
                          <tr key={ingredient.id} className="border-b border-border last:border-0">
                            ...
                            <td className="px-4 py-3">{(ingredient.baseWeightGrams * scaleFactor).toFixed(1)}g</td>
                            <td className="px-4 py-3">{ingredient.bakersPercent.toFixed(1)}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
